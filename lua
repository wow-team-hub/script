...
-- Table : chemins et options par placeId
local PlaceConfig = {
    [3311165597] = {
        path = "Package.Events.letsplayagame",
        useB = false,
        handshakeToken = "__handshake__"
    },
    [5151400895] = {
        path = "Package.Events.b.Dece",
        useB = false,
        handshakeToken = "__handshake__"
    }
}
...
--// UTILITAIRES
local function getInstanceFromPath(root, path)
    local current = root
...
local function getAttackRemote()
    local inst = getInstanceFromPath(ReplicatedStorage, config.path)
    if not inst then return nil end
    if not (inst:IsA("RemoteFunction") or inst:IsA("RemoteEvent")) then
        return nil
    end
    return inst
end

local function doHandshake(remote)
    if not remote or not remote:IsA("RemoteFunction") then return true end
    local ok, resp = pcall(function()
        return remote:InvokeServer(config.handshakeToken)
    end)
    return ok and resp == "ok"
end
...
local function sendDiscordAlert(message)
    ...
    if not success then
        warn("Échec envoi webhook Discord :", response)
    end
end

--// RÉCUPÉRATION DES REMOTES
local attackRemote = getAttackRemote()
local scriptStopped = false

if not attackRemote then
    local msg = string.format(
        "Le chemin **%s** est invalide ou introuvable dans ReplicatedStorage.\n" ..
        "Le script ne peut pas fonctionner.\n" ..
        "⚠️ Please do not use fast auto skills at this time. ! ⚠️",
...
if not doHandshake(attackRemote) then
    warn("Handshake échoué, script stoppé.")
    return
end
